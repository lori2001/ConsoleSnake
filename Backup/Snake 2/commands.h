...
void snakesound()
{
 if ((clock() - last_time) >= (random_between(2,4) * CLOCKS_PER_SEC) &&
     (clock() - eat_time ) >= (         1          * CLOCKS_PER_SEC) && soundstatus == 1 )
 {
  last_time = clock();
...
void deallocate()
{
    ...
    for (y = 0; y < MAX_Y ; y++)
    {
     ...
     stprepeat = obstnum = anim = foodnum = coins = score = hscore = snakelong1 = newhiscore = score_verifier = 0 ;
}
